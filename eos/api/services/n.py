    ...
    send_data = select_one("""SELECT
                        json_build_object('type', 'FeatureCollection', 'features', 
                            json_agg(ST_AsGeoJson(ST_Transform(shape, 4326))::jsonb || jsonb_build_object('properties',
                             jsonb_build_object('id', feature_data->'id', 'pid', polygon_id, 'crop_type', {}) ))
                        ) as res
                    FROM eos_final.api_polygons 
                    WHERE fermer_id={} {};""".format(data['crop_type'], data["fermer_id"], add_query))
...
def area_stat(request):
    ...
    if len(query_data) > 0:
        ...
        return JsonResponse(stat_request.json(), safe=False)from rest_framework.decorators import api_view
from ..modules.query import select_many, select_one
from ..modules.config import ATTR, HEADERS
from django.http import JsonResponse
import requests
